  ...
  return (
    <section id="references" className="min-h-screen bg-[#0d0d0d] text-[#f0f0e8] py-32 px-6 md:px-12 noise-bg relative">
      <div className="max-w-6xl mx-auto">
        {/* Section Header */}
        <div className="mb-20">
          <p className="font-mono text-[#00f0ff] text-sm tracking-[0.3em] uppercase mb-4">Testimonials</p>
          <h2 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {references.map((ref, idx) => (
            ...
            >
              {/* Large quote mark */}
              <div className="absolute top-4 left-6 font-display text-8xl text-[#00f0ff] opacity-10 leading-none select-none">
                "
              </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-[#00f0ff] font-mono text-sm">{ref.role}</p>
                  {ref.company && (
                    <p className="text-[#666] text-sm mt-1">{ref.company}</p>
                  )}
                  <p className="text-[#666] font-mono text-xs mt-2">{ref.date}</p>
                </div>

                {/* Download Button */}
                <button
                  onClick={() => handleDownload(ref)}
                  className="w-12 h-12 border-2 border-[#2a2a2a] flex items-center justify-center hover:border-[#00f0ff] hover:bg-[#00f0ff] hover:text-[#0d0d0d] transition-all duration-300 flex-shrink-0"
